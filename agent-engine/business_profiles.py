...
class BusinessManager:
    """Manage business profiles, catalogs, and orders with file-based persistence."""

    _DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "business_data.json")

    def __init__(self):
        self.profiles: Dict[str, BusinessProfile] = {}
        self.catalogs: Dict[str, List[CatalogItem]] = {}
        self.orders: Dict[str, List[Order]] = {}
        self._load()

    def _load(self):
        """Load data from JSON file (survives server restarts)."""
        try:
            if os.path.exists(self._DATA_FILE):
                ...
                for bid, items in data.get("catalogs", {}).items():
                    self.catalogs[bid] = [CatalogItem(**i) for i in items]
                for bid, order_list in data.get("orders", {}).items():
                    self.orders[bid] = [Order(**o) for o in order_list]
                logger.info(f"[v] Business data loaded from {self._DATA_FILE}")
        except Exception as e:
    ...
    def _save(self):
        ...
        try:
            data = {
                ...
                "catalogs": {bid: [i.to_dict() for i in items] for bid, items in self.catalogs.items()},
                "orders": {bid: [o.to_dict() for o in order_list] for bid, order_list in self.orders.items()},
            }
            with open(self._DATA_FILE, "w", encoding="utf-8") as f:
                json.dump(data, f, default=str)
            logger.info(f"[+] Business data saved to {self._DATA_FILE}")
        except Exception as e:
            logger.error(f"[!] Failed to save business data: {e}")
