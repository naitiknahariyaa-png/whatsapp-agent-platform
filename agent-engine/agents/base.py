...
class BaseAgent:
    ...
    async def run(self, user_input: str, context: Dict[str, Any] = None) -> str:
        ...
        # Business facts get their own authoritative block so specialists
        ...
        messages = [
            {"role": "system", "content": self._build_system_prompt(knowledge_context) + biz_block},
            {"role": "user", "content": user_input}
        ]
        if context:
            ctx_wo_biz = {k: v for k, v in context.items() if k != "business_context"}
            messages.append({"role": "system", "content": f"Context: {json.dumps(ctx_wo_biz)}"})
            ...
            if tool_call:
                ...
                if tool_name in self.tools:
                    ...
                    if isinstance(result, dict) and result.get("status") == "error":
                        # Tell the agent the tool failed and to try differently
                        # instead of crashing the turn with a raw exception.
                        obs = (
                            f"Observation: The tool '{tool_name}' could not be completed. "
                            f"Error: {result.get('error', result.get('message', 'unknown'))}. "
                            f"Do NOT retry the same failing tool. Try a different tool, "
                            f"ask the user for clarification, or answer without the tool."
                        )
                    else:
